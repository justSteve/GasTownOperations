...
**Key Paths**:
- Source repo: `/root/gastown` (remotes: `origin`=justSteve, `upstream`=steveyegge)
...
Source it: `tmux source-file /root/gtOps/daemon/gt-tmux.conf`
...
| `/root/gtOps/daemon/gt-tmux.conf` | TMux keybindings |
| `/root/gtOps/daemon/tmux_controller.py` | TMux automation library |
| `/root/gtOps/daemon/vscode_bridge.py` | VSCode automation |
| `/root/gtOps/scripts/gt-sync.sh` | Daily GT update script |
| `/root/gtOps/.claude/commands/gt-update.md` | Slash command for updates |
